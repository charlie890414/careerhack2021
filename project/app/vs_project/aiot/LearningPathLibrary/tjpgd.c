...
)
{
	...
	nd = (uint16_t)((nd + 3) & ~3);			/* Align block size to the word boundary */
...
)
{
	...
	nby = (uint16_t)(jd->msx * jd->msy);	/* Number of Y blocks (1, 2 or 4) */
	nbc = 2;					/* Number of C blocks (2) */
	bp = jd->mcubuf;			/* Pointer to the first block */

	for (blk = 0; blk < nby + nbc; blk++) {
		cmp = (uint16_t)((blk < nby) ? 0 : blk - nby + 1);	/* Component number 0:Y, 1:Cb, 2:Cr */
...
)
{
	...
	mx = (uint16_t)(jd->msx * 8); my = (uint16_t)(jd->msy * 8);					/* MCU size (pixel) */
	rx = (uint16_t)((x + mx <= jd->width) ? mx : jd->width - x);	/* Output rectangular size (it may be clipped at right/bottom end) */
	ry = (uint16_t)((y + my <= jd->height) ? my : jd->height - y);
	if (JD_USE_SCALE) {
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	/* Not for 1/8 scaling */
		...
		for (iy = 0; iy < my; iy++) {
			...
			for (ix = 0; ix < mx; ix++) {
				cb = (int16_t)(pc[0] - 128); 	/* Get Cb/Cr component and restore right level */
				cr = (int16_t)(pc[64] - 128);
		...
		if (JD_USE_SCALE && jd->scale) {
			...
			s = (uint16_t)(jd->scale * 2);	/* Bumber of shifts for averaging */
			w = (uint16_t)(1 << jd->scale);	/* Width of square */
			a = (uint16_t)((mx - w) * 3);	/* Bytes to skip for next line in the square */
...
)
{
	...
	for (;;) {
		...
		case 0xDA:	/* SOS */
			...
			n = (uint16_t)(jd->msy * jd->msx);						/* Number of Y blocks in the MCU */
			if (!n) return JDR_FMT1;					/* Err: SOF0 has not been loaded */
			len = (uint16_t)(n * 64 * 2 + 64);						/* Allocate buffer for IDCT and RGB output */
...
)
{
	...
	mx = (uint16_t)(jd->msx * 8); (uint16_t)(my = jd->msy * 8);			/* Size of the MCU (pixel) */
