...
static pthread_spinlock_t update_lock;
static int count = 0;
...
/// <summary>
/// 	handle the response of model
/// </summary>
static void SocketEventHandler(EventData *eventData)
{
	uint8_t buf[5];

	ssize_t bytesReceived = recv(rtSocketFd, &buf[0], sizeof(buf), 0);
	if (bytesReceived < 0)
	{
	...
	index = buf[0];
	exec_time = ((buf[4] << 24) | (buf[3] << 16) | (buf[2] << 8) | buf[1]);
	count = (count + 1) % 8;
...
/// <summary>
/// 	create comunication thread for RT core
/// </summary>
...
int main(int argc, char *argv[])
{
	...
	Log_Debug("Start Init Socket to Inter Core\r\n");
	...
	while (!lp_isTerminationRequired())
	{
		...
		{
			++sentMsgCount;
			Log_Debug("Inference Result Received[%d]: %s.\r\n", sentMsgCount, label[index]);
		}
